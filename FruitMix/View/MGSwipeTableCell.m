...
#pragma _panRecognizer delegate 
...
//jy_contentView initial
-(UIView *)jy_contentView{
    if (!_jy_contentView) {
        _jy_contentView = [[UIView alloc]initWithFrame:CGRectMake(JY_ALBUM_CELL_LEFT_PADDING, JY_ALBUM_CELL_LEFT_PADDING/2, self.contentView.bounds.size.width-2*JY_ALBUM_CELL_LEFT_PADDING, self.contentView.bounds.size.height-JY_ALBUM_CELL_LEFT_PADDING)];
        _jy_contentView.backgroundColor = [UIColor whiteColor];
        _jy_contentView.layer.cornerRadius = 2;
        _jy_contentView.layer.masksToBounds = YES;
        [self.contentView addSubview:_jy_contentView];
    }
    return _jy_contentView;
}
...
-(UIView *) swipeContentView
{
    if (!_swipeContentView) {
        _swipeContentView = [[UIView alloc] initWithFrame:self.jy_contentView.bounds];
        ...
        [self.jy_contentView addSubview:_swipeContentView];
    }
...
-(void) layoutSubviews
{
    [super layoutSubviews];
    if (_swipeContentView) {
        _swipeContentView.frame = self.jy_contentView.bounds;
    }
    if (_swipeOverlay) {
        CGSize prevSize = _swipeView.bounds.size;
        _swipeOverlay.frame = CGRectMake(0, 0, self.jy_contentView.bounds.size.width, self.jy_contentView.bounds.size.height);
    ...
    //resize contentView
    if (_jy_contentView) {
        _jy_contentView.frame = CGRectMake(JY_ALBUM_CELL_LEFT_PADDING, JY_ALBUM_CELL_LEFT_PADDING/2, self.contentView.bounds.size.width-2*JY_ALBUM_CELL_LEFT_PADDING, self.contentView.bounds.size.height-JY_ALBUM_CELL_LEFT_PADDING);
    }
...
{
    if (!_swipeOverlay) {
        _swipeOverlay = [[UIView alloc] initWithFrame:CGRectMake(0, 0, self.jy_contentView.bounds.size.width, self.jy_contentView.bounds.size.height)];
        [self fixRegionAndAccesoryViews];
        ...
        [self.jy_contentView addSubview:_swipeOverlay];
    }
...
-(void) setAccesoryViewsHidden: (BOOL) hidden
{
    ...
    for (UIView * view in self.jy_contentView.subviews) {
...
-(UIColor *) backgroundColorForSwipe
{
    ...
    else if (self.jy_contentView.backgroundColor && ![self.jy_contentView.backgroundColor isEqual:[UIColor clearColor]]) {
        return self.jy_contentView.backgroundColor;
    }
