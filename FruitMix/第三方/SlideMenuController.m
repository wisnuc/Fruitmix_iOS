...
-(UIInterfaceOrientationMask)supportedInterfaceOrientations {
    UIViewController * vc = [FMUtil getTopViewController];
    if ([vc isKindOfClass:[IDMPhotoBrowser class]]) {
        return UIInterfaceOrientationMaskAll;
    }
    else if (!_mainViewController == nil) {
    ...
    switch (panGesture.state) {
        case UIGestureRecognizerStateBegan: {
            ...
            if ([self isLeftHidden]) {
                ...
                if (point.x<options.leftViewWitdth - 50) {
                    return;
                }
            }
            
            if (LPSLastState != UIGestureRecognizerStateEnded) {
                LPSLastState = UIGestureRecognizerStateEnded;//jy
                return;
            }
...
//        UIViewController *vc2 = [(RDVTabBarController *)vc selectedViewController];
        ...
        [vc presentViewController:newMainController animated:YES completion:nil];
...
//                [vc3 presentViewController:newMainController animated:YES completion:nil];
//            }
//        }
...
-(struct PanInfo)panLeftResultInfoForVelocity:(CGPoint)velocity {
    //jy
    CGFloat thresholdVelocity = 500.0;
...
-(BOOL)gestureRecognizer:(UIGestureRecognizer *)gestureRecognizer shouldReceiveTouch:(UITouch *)touch {
    ...
    if (gestureRecognizer == _leftPanGesture) {
        shouldRec = [self slideLeftForGestureRecognizer:gestureRecognizer inPoint:point];
        return shouldRec;
    } else if (gestureRecognizer == _rightPanGesture) {
        shouldRec = [self slideRightViewForGestureRecognizer:gestureRecognizer withTouchPoint:point];
       return shouldRec;
    } else if (gestureRecognizer == _leftTapGesture) {
        shouldRec = [self isLeftOpen] && ![self isPointContainedWithinLeftRect:point];
        return shouldRec;
    } else if (gestureRecognizer == _rightTapGesture) {
        shouldRec = [self isRightOpen] && ![self isPointContainedWithinRightRect:point];
        return shouldRec;
    }
    return NO;
}
