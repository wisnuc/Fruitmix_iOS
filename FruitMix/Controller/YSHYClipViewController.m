...
-(instancetype)initWithPhoto:(id<IDMPhoto>)photo{
    if(self = [super init])
    {
        if ([photo underlyingImage]) {
            _image = [self fixOrientation:[photo underlyingImage]];
        }else{
            [photo loadUnderlyingImageAndNotify];
            _image = [photo placeholderImage];
            
            [SXLoadingView showProgressHUD:@"正在加载大图"];
...
-(void)CreatUI
{
    ...
    [clipBtn setFrame:CGRectMake(0, self.view.frame.size.height - 50 - 64, self.view.frame.size.width, 50)];
...
-(void)drawClipPath:(ClipType )clipType
{
    ...
    CGPoint center = self.view.center;
     ...
     self.circularFrame = CGRectMake(center.x - self.radius, center.y - self.radius - 32, self.radius * 2, self.radius * 2);
    UIBezierPath * path= [UIBezierPath bezierPathWithRect:CGRectMake(0, 0, ScreenWidth, ScreenHeight)];
     CAShapeLayer *layer = [CAShapeLayer layer];
    if(clipType == CIRCULARCLIP)
    {
        //绘制圆形裁剪区域
        [path  appendPath:[UIBezierPath bezierPathWithArcCenter:CGPointMake(center.x, center.y-32) radius:self.radius startAngle:0 endAngle:2*M_PI clockwise:NO]];
    }
    else
    {
...
-(void)handlePinGesture:(UIPinchGestureRecognizer *)pinGesture
{
    ...
    else if(pinGesture.state == UIGestureRecognizerStateEnded)
    {
        ...
        [view setCenter:CGPointMake(self.view.center.x, self.view.center.y - 32)];
